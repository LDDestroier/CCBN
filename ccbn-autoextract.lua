		...
		if info.direction == 1 then\\\
			...
			if info.y > 1 then\\\
				if info.x <= 6 then\\\
					...
					info.y = info.y - (3 / stage.panelHeight)\\\
				end\\\
...
return {\\\
    ...
    [ \".ccbn-data/chipdata/icestage\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"IceStage\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/singlebomb\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"SingleBomb\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/sonicwave\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"SonicWave\\\",\\\
		description = \\\"Piercing ground wave!\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/triplebomb\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"TripleBomb\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/cannonball\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"CannonBall\\\",\\\
    ...
    [ \".ccbn-data/chipdata/bigwave\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"BigWave\\\",\\\
		description = \\\"Piercing ground wave that cracks panels!\\\",\\\
		cooldown = {\\\
			shoot = 14,\\\
			move = 8\\\
		}\\\
	...
	logic = function(info)\\\
		...
		info.x = info.x + (4 / stage.panelWidth) * info.direction\\\
\\\
		act.stage.setDamage(info.x, info.y, 190, info.owner, 10, false, {}, info.safeObjects)\\\
		if not info.safePanels[math.floor(info.x + 0.5)] then\\\
			act.stage.crackPanel(info.x, info.y, 1)\\\
			info.safePanels[math.floor(info.x + 0.5)] = true\\\
		end\\\
\\\
		if info.frame > 50 or not act.stage.checkIfSolid(info.x, info.y) then\\\
			return false\\\
    ...
    [ \".ccbn-data/chipdata/wideshot\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"WideShot\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/lavaball\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"LavaBall\\\",\\\
		description = \\\"Lob a lava ball 2 panels forward!\\\",\\\
		cooldown = {\\\
			shoot = 6,\\\
			move = 5\\\
		}\\\
	...
	logic = function(info)\\\
		...
		local parabola = math.sin((math.pi / maxFrames) * info.frame) * 2\\\
		if parabola < 0.1 and info.frame > 3 then\\\
			act.stage.setDamage(info.x, info.y, 120, info.owner, 1, false)\\\
			act.stage.setType(info.x, info.y, \\\"lava\\\")\\\
			return false\\\
		else\\\
			info.x = info.x + (maxDist / maxFrames) * info.direction\\\
		end\\\
...
return {\\\
    ...
    [ \".ccbn-data/chipdata/dynawave\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"DynaWave\\\",\\\
		description = \\\"Piercing ground wave!\\\",\\\
	...
	logic = function(info)\\\
		...
		info.x = info.x + (4 / stage.panelWidth) * info.direction\\\
\\\
		act.stage.setDamage(info.x, info.y, 100, info.owner, 10, false, {}, info.safeObjects)\\\
\\\
		if info.frame > 50 or not act.stage.checkIfSolid(info.x, info.y) then\\\
			return false\\\
		else\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/iceball\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"IceBall\\\",\\\
		description = \\\"Lob a freezing ball 2 panels forward!\\\",\\\
		cooldown = {\\\
			shoot = 6,\\\
			move = 5\\\
		}\\\
	...
	logic = function(info)\\\
		...
		if parabola < 0.1 and info.frame > 3 then\\\
			act.stage.setDamage(info.x, info.y, 120, info.owner, 1, false)\\\
			act.stage.setType(info.x, info.y, \\\"ice\\\")\\\
			return false\\\
		else\\\
			info.x = info.x + (maxDist / maxFrames) * info.direction\\\
		end\\\
...
return {\\\
    ...
    [ \".ccbn-data/chipdata/crackshot\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"CrackShot\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/geddon3\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"Geddon3\\\",\\\
		description = \\\"Turns all panels into poison!\\\",\\\
		cooldown = {\\\
			shoot = 8,\\\
			move = 4\\\
		}\\\
...
\\\
    ...
    [ \".ccbn-data/objectdata/rockcube\" ] = \"local stage, players, objects, projectiles, act, images, game = ...\\\
return {\\\
    ...
    [ \".ccbn-data/chipdata/fastgauge\" ] = \"local stage, players, objects, projectiles, act, images, game = ...\\\
return {\\\
	info = {\\\
		name = \\\"FastGauge\\\",\\\
		description = \\\"Doubles custom gauge speed!\\\",\\\
		cooldown = {\\\
			shoot = 6,\\\
			move = 2\\\
		}\\\
    ...
    [ \".ccbn-data/chipdata/boomer2\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"Boomer2\\\",\\\
		description = \\\"Boomerang that orbits stage!\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/fullcustom\" ] = \"local stage, players, objects, projectiles, act, images, game = ...\\\
return {\\\
	info = {\\\
		name = \\\"FullCustom\\\",\\\
		description = \\\"Instantly fills custom bar!\\\",\\\
		cooldown = {\\\
			shoot = 6,\\\
			move = 2\\\
		}\\\
	...
	logic = function(info)\\\
		game.custom = game.customMax\\\
		return false\\\
	end,\\\
}\",\
    [ \".ccbn-data/chipdata/prism\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"Prism\\\",\\\
    ...
    [ \".ccbn-data/chipdata/doublebomb\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"DoubleBomb\\\",\\\
    ...
    [ \".ccbn-data/objectdata/prism\" ] = \"local stage, players, objects, projectiles, act, images, game = ...\\\
return {\\\
	image = images.prism,\\\
	friendlyFire = true,		-- can it hit its owner?\\\
	health = 500,				-- amount of damage before disintegrating\\\
...
return {\\\
	...
	logic = function(info)\\\
		if info.frame == 0 then\\\
			...
			if info.frame == 7 * 2 + 1 or info.player.cooldown.struck ~= 0 then\\\
				info.player.canMove = true\\\
				info.player.canShoot = true\\\
				info.player.cooldown.shoot = math.max(6, info.player.cooldown.shoot)\\\
				info.player.cooldown.move = 4\\\
...
return {\\\
	...
	logic = function(info)\\\
		if info.frame == 0 then\\\
			...
			if info.frame == 3 * 2 + 1 or info.player.cooldown.struck ~= 0 then\\\
				info.player.canMove = true\\\
				info.player.canShoot = true\\\
				info.player.cooldown.shoot = math.max(6, info.player.cooldown.shoot)\\\
				info.player.cooldown.move = 4\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/sandstage\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"SandStage\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/boomer3\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"Boomer3\\\",\\\
		description = \\\"Boomerang that orbits stage!\\\",\\\
...
\\\
		act.stage.setDamage(info.x, info.y, 100, info.owner, 2, false, {}, info.safeObjects)\\\
		return true, {{images.boomer, info.x, info.y, nil, info.frame % 2 * 2 - 1}}\\\
	end\\\
}\",\
...
return {\\\
	...
	logic = function(info)\\\
		if info.frame == 0 then\\\
			...
			if info.frame == 12 * 2 + 1 or info.player.cooldown.struck ~= 0 then\\\
				info.player.canMove = true\\\
				info.player.canShoot = true\\\
				info.player.cooldown.shoot = math.max(6, info.player.cooldown.shoot)\\\
				info.player.cooldown.shoot = 4\\\
				return false\\\
			elseif info.frame % 2 == 0 then\\\
				act.projectile.newProjectile(info.x, info.y, info.player, \\\"vulcan_internal\\\", info.frame < 12 * 2)\\\
			end\\\
		end\\\
		return true\\\
	end\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/lavastage\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"LavaStage\\\",\\\
...
\\\
    ...
    [ \".ccbn-data/chipdata/repair\" ] = \"local stage, players, objects, projectiles, act, images = ...\\\
return {\\\
	info = {\\\
		name = \\\"Repair\\\",\\\
...
return {\\\
    ...
    [ \".ccbn-data/chipdata/slowgauge\" ] = \"local stage, players, objects, projectiles, act, images, game = ...\\\
return {\\\
	info = {\\\
		name = \\\"SlowGauge\\\",\\\
		description = \\\"Halves custom gauge speed!\\\",\\\
		cooldown = {\\\
			shoot = 6,\\\
			move = 2\\\
		}\\\
...
local howToPlay = function()\\\
	local help = {\\\
		...
		\\\" you fucking die. To help with that,\\\",\\\
